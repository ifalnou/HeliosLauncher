...
function resolveSelectedRAM(ram) {
    if(ram?.recommended != null) {
        return `${Math.floor(ram.recommended)}M`
    } else {
        // Legacy behavior
        const mem = os.totalmem()
...
exports.getMinRAM = function(serverid){
    let ram = config.javaConfig[serverid].minRAM;
    if (ram.endsWith('M')) ram = ram.match(/^([0-9]+)/)[1]+'M';
    return ram;
}
...
exports.getMaxRAM = function(serverid){
    let ram = config.javaConfig[serverid].maxRAM;
    if (ram.endsWith('M')) ram = ram.match(/^([0-9]+)/)[1]+'M';
    return ram;
}
